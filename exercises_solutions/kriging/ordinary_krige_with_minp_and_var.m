function [z, kv]=ordinary_krige_with_minp_and_var(xi, yi, zi, x, y, variogram, range, sill, nugget, min_p, max_p)
  ...
  % the weighted mean z and the Kriging variance kv.
  %
  % Parameters
  % ----------
  ...
  % the kriging variance is the dot product of the weights and the
  % semivariances
  kv = dot(w(1:n), B(1:n));
end
